...
void SceneDocument::setElementRef(int gi, int ei,
                                   const std::string& maskId,
                                   const std::string& parentId)
{
    if (gi < 0 || gi >= (int)m_scene.graphics.size()) return;
    const std::string& gid = m_scene.graphics[gi].id;
    if (ei < 0 || ei >= (int)m_scene.graphics[gi].elements.size()) return;
    const std::string& eid = m_scene.graphics[gi].elements[ei].id;

    m_elementRefs[gid][eid] = {maskId, parentId};
...
// static
nlohmann::json SceneDocument::sceneToJson(const Scene& scene,
