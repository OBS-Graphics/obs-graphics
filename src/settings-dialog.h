...
struct DataSourceRow {
    ...
    QString path;          // full path; display-only, not the pool key. Empty for a source with no file (e.g. manual).
    QString name;          // display name: GetDisplayName(), or a path-derived fallback
    bool reloadable{true}; // false for a source with no file behind it — reloading it is meaningless
