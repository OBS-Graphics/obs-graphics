...
- `app-config.h/cpp` — reads/writes plugin config (per-title entries, data-source pool entries) to a per-profile/scene-collection JSON file. Schema v7: a `DataSourceEntry` carries `kind` (`"file"` or `"manual"`); a manual entry has no `path` and instead stores `name`, `columns` (element id + `"text"`/`"image"`), and `rows` (kept ragged verbatim — a row may be shorter or longer than `columns`).
...
- `title-settings-dialog.h/cpp` — per-title dialog: a combo box pointing the title at one of the pool's source ids, the resulting record table/selection, the auto-hide duration, and the "New manual data source..."/"Edit manual data..." buttons (emits requests; the dock does the actual pool mutation)
- `manual-source-dialog.h/cpp` — editor for one `ManualDataSource`'s table (name, columns, records), opened by the dock from the per-title dialog's two manual-source buttons; owns no pool/scene state, does no I/O beyond its own image-path file picker
...
- **A `ManualDataSource` has no file behind it** — `GetFilePath()` is always empty. `checkDataSourceHealth()` treats an empty path as healthy rather than checking existence (a file-existence check is meaningless with no file), and the Data Sources tab hides its Reload button for the same reason. Its editor (`manual-source-dialog.h`) is reached only from the per-title dialog's "New manual data source..."/"Edit manual data..." buttons, never from the app-level "Add Data Source..." file picker. After Create/Edit, `graphics-dock.cpp` calls `Pool().DataBlocking(id)` with no scene lock held before refreshing the UI: for a `ManualDataSource` that's the inherited default (`GetData()`), so it returns instantly, but it still bumps the pool's cache version immediately — without it the just-saved edit would keep showing the pool's previous cache for up to one 0.25s poll interval.
